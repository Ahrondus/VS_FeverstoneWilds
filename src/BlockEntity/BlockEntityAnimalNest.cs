...
namespace FeverstoneWilds
{
    ...
    public class BlockEntityAnimalNest : BlockEntity, IAnimalNest
    {
        ...
        {
            return entity is EntityAgent && new string []{"ostrich-female"}.Contains(entity.Code.Path);
        }
        ...
        public bool TryAddEgg(Entity entity, string chickCode, double incubationTime)
        {
            ...
            eggs++;
            Block replacementBlock = Api.World.GetBlock(new AssetLocation("ostrich",Block.FirstCodePart() + "-" + eggs + (eggs > 1 ? "eggs" : "egg")));
        ...
        private void On1500msTick(float dt)
        {
            ...
            if (timeToIncubate <= 0)
            {
                ...
                Block replacementBlock = Api.World.GetBlock(new AssetLocation("ostrich",Block.FirstCodePart() + "-empty"));
                Api.World.BlockAccessor.ExchangeBlock(replacementBlock.Id, this.Pos);
